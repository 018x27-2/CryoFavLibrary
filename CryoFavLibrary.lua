...
function Library:MakeDraggable(Instance, Cutoff)
    local frame = Instance -- Assuming 'Instance' is your GUI element
...
frame.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then -- Check if left mouse button is released
        dragToggle = false
    end
end)

UIS.InputChanged:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseMovement then -- Check if mouse is moved
        if dragToggle then
            updateInput(input)
        end
    end
